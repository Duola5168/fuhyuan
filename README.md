...
*   **🚀 彈性的 PDF 產生與傳送**：
    ...
    *   可選擇將產生的 PDF 報告上傳至指定的 **NAS 伺服器**，或透過 **Email (使用 Brevo)** 寄送，亦可兩者同時進行。
    *   可直接下載 PDF 或使用 Web Share API 進行分享。
*   **🔒 安全第一**：所有 API 金鑰與伺服器憑證等敏感資訊，皆採用業界標準的環境變數進行管理，絕不外洩於程式碼中。
...
3.  **將您的秘密金鑰新增至 `.env.local`**：
    ...
    # QNAP NAS 上傳功能所需憑證
    # 請填寫您 QNAP NAS 的相關資訊
    NAS_HOST="您的 NAS 的 IP 位址或網域名稱"
    NAS_PORT="8080" # 您的 NAS 的通訊埠
    NAS_USERNAME="您的 NAS 登入帳號"
    NAS_PASSWORD="您的 NAS 登入密碼"
    UPLOAD_PATH="/Public/Uploads" # 您希望檔案上傳到 NAS 的哪個共用資料夾路徑
    ```
